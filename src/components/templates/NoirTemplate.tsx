    ...
    <div className="bg-[#0A0A0A] aspect-[210/297] font-sans text-[#F5F5F5] border-t-[2px] border-t-[#C4977A] border-b-[2px] border-b-[#C4977A] flex flex-col">
      ...
      <div className="bg-[#0A0A0A] pt-6 pb-4 px-4 flex justify-between items-start">
        <div className="flex flex-col items-start">
          <div className="text-[24px] font-[100] tracking-[6px] text-[#F5F5F5] italic leading-none mb-2">STATEMENT</div>
      ...
      <div className="py-3 px-4 flex justify-between items-stretch gap-3">
        <div className="w-1/2 flex flex-col justify-between">
          <div>
            ...
            <div className="text-[14px] font-[300] text-[#F5F0E8] tracking-[0.5px]">{invoice.client_name}</div>
        ...
        <div className="w-1/2 text-right flex flex-col justify-between">
          <div>
            ...
            <div className="text-[12px] text-[#F5F0E8] capitalize">{invoice.status}</div>
          </div>
          <div className="mt-1.5">
            <div className="text-[8px] text-[#888888] uppercase tracking-widest mb-0.5">Total Remittance</div>
            <div className="text-[24px] font-bold text-[#F5F0E8] tracking-[-0.5px] leading-none">{formatCurrency(invoice.total_amount)}</div>
          </div>
        </div>
      </div>
        ...
        <div className="mb-2">
          {items.map((item, i) => (
            <div key={i} className="py-1.5 border-b border-[#1A1A1A] flex justify-between items-center">
              <div>
                <div className="text-[12px] font-[300] text-[#F5F0E8] tracking-[0.5px]">{item.description}</div>
                <div className="text-[9px] text-[#C4977A] italic mt-0.5">Bespoke Service</div>
              </div>
              <div className="text-right">
                <div className="text-[12px] text-[#F5F0E8] font-bold">{formatCurrency(item.quantity * item.unit_price)}</div>
        ...
        <div className="mt-2 pt-2 border-t border-[#1A1A1A] flex justify-between items-start">
          ...
          <div className="w-1/2 pl-3 flex flex-col items-end space-y-1">
            <div className="flex justify-between w-full max-w-[160px]">
              ...
              <span className="text-[#F5F0E8] text-[10px]">{formatCurrency(invoice.subtotal)}</span>
            </div>
            <div className="flex justify-between w-full max-w-[160px]">
              <span className="text-[#888888] text-[10px]">Tax</span>
              <span className="text-[#F5F0E8] text-[10px]">{formatCurrency(invoice.tax_amount)}</span>
            </div>
            <div className="flex justify-between w-full max-w-[160px] pt-1 border-t border-[#1A1A1A]">
              ...
              <span className="text-[#F5F0E8] font-bold text-[13px]">{formatCurrency(invoice.total_amount)}</span>
            </div>
          </div>
        </div>
